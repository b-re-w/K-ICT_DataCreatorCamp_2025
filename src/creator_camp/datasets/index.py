...
class SentinelIndex(Enum):
    ...
    @classmethod
    def get_partial_train_urls(cls):
        return [
            SentinelIndex.TRAIN.url.replace(".zip", f"_p{i}.zip") for i in range(1, 9)
        ]

    @classmethod
    def get_partial_trains(cls):
        return [
            SentinelIndex.TRAIN.value.replace(".zip", f"_p{i}.zip") for i in range(1, 9)
        ]
