...
class RTDetrDecoderOutput(ModelOutput):
    ...
    intermediate_predicted_corners (`torch.FloatTensor` of shape `(batch_size, config.decoder_layers, num_queries, 5)`):
        Stacked intermediate predicted corners (predicted corners of each layer of the decoder).
    initial_reference_points (`torch.FloatTensor` of shape `(batch_size, config.decoder_layers, num_queries, 5)`):
...
class RTDetrModelOutput(ModelOutput):
    ...
    intermediate_logits (`torch.FloatTensor` of shape `(batch_size, config.decoder_layers, sequence_length, config.num_labels)`):
        Stacked intermediate logits (logits of each layer of the decoder).
    intermediate_reference_points (`torch.FloatTensor` of shape `(batch_size, config.decoder_layers, num_queries, 5)`):
        Stacked intermediate reference points (reference points of each layer of the decoder).
    intermediate_predicted_corners (`torch.FloatTensor` of shape `(batch_size, config.decoder_layers, num_queries, 5)`):
        Stacked intermediate predicted corners (predicted corners of each layer of the decoder).
    initial_reference_points (`torch.FloatTensor` of shape `(batch_size, num_queries, 5)`):
        Initial reference points used for the first decoder layer.
    init_reference_points (`torch.FloatTensor` of shape `(batch_size, num_queries, 5)`):
    ...
    enc_topk_bboxes (`torch.FloatTensor` of shape `(batch_size, sequence_length, 5)`):
        Logits of predicted bounding boxes coordinates in the encoder stage.
    enc_outputs_class (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.num_labels)`, *optional*, returned when `config.with_box_refine=True` and `config.two_stage=True`):
        Predicted bounding boxes scores where the top `config.two_stage_num_proposals` scoring bounding boxes are
        picked as region proposals in the first stage. Output of bounding box binary classification (i.e.
        foreground and background).
    enc_outputs_coord_logits (`torch.FloatTensor` of shape `(batch_size, sequence_length, 5)`, *optional*, returned when `config.with_box_refine=True` and `config.two_stage=True`):
...
class RTDetrObjectDetectionOutput(ModelOutput):
    ...
    pred_boxes (`torch.FloatTensor` of shape `(batch_size, num_queries, 5)`):
    ...
    enc_topk_bboxes (`torch.FloatTensor` of shape `(batch_size, sequence_length, 5)`, *optional*, returned when `config.with_box_refine=True` and `config.two_stage=True`):
        Logits of predicted bounding boxes coordinates in the encoder.
    enc_outputs_class (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.num_labels)`, *optional*, returned when `config.with_box_refine=True` and `config.two_stage=True`):
        Predicted bounding boxes scores where the top `config.two_stage_num_proposals` scoring bounding boxes are
        picked as region proposals in the first stage. Output of bounding box binary classification (i.e.
...
def get_contrastive_denoising_training_group(
    ...
    input_query_bbox = torch.zeros([batch_size, max_gt_num, 5], device=device)
    ...
    if box_noise_scale > 0:
        # 5D bbox에 대한 노이즈 처리
        # cx, cy, w, h만 corners format으로 변환 (length는 별도 처리)
        bbox_4d = input_query_bbox[..., :4]  # cx, cy, w, h
        length = input_query_bbox[..., 4:5]   # length

        known_bbox = center_to_corners_format(bbox_4d)
        diff = torch.tile(bbox_4d[..., 2:] * 0.5, [1, 1, 2]) * box_noise_scale  # w, h만 사용
        ...
        bbox_4d_noisy = corners_to_center_format(known_bbox)

        # length에 대한 노이즈 추가 (별도 처리)
        length_diff = length * 0.5 * box_noise_scale
        length_rand_sign = torch.randint_like(length, 0, 2) * 2.0 - 1.0
        length_rand_part = torch.rand_like(length)
        length_rand_part = (length_rand_part + 1.0) * negative_gt_mask + length_rand_part * (1 - negative_gt_mask)
        length_rand_part *= length_rand_sign
        length_noisy = length + length_rand_part * length_diff
        length_noisy = torch.clamp(length_noisy, min=0.0, max=1.0)  # length도 0-1 범위로 제한

        # 4D bbox와 length 결합하여 5D bbox 생성
        input_query_bbox = torch.cat([bbox_4d_noisy, length_noisy], dim=-1)
...
class RTDetrMultiscaleDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        if num_coordinates == 2:
            ...
            sampling_locations = (
                reference_points[:, :, None, :, None, :2]
                + sampling_offsets / self.n_points * reference_points[:, :, None, :, None, 2:4] * 0.5  # width, height만 사용
            )
        else:
            raise ValueError(f"Last dim of reference_points must be 2, 4 or 5, but got {reference_points.shape[-1]}")
...
class RTDetrDecoder(RTDetrPreTrainedModel):
    def __init__(self, config: RTDetrConfig):
        ...
        self.query_pos_head = RTDetrMLPPredictionHead(config, 5, 2 * config.d_model, config.d_model, num_layers=2)
...
class RTDetrModel(RTDetrPreTrainedModel):
    def __init__(self, config: RTDetrConfig):
        ...
        self.enc_bbox_head = RTDetrMLPPredictionHead(config, config.d_model, config.d_model, 5, num_layers=3)
    ...
    def generate_anchors(self, spatial_shapes=None, grid_size=0.05, device="cpu", dtype=torch.float32):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            length = torch.ones_like(grid_xy[..., :1]) * grid_size * (2.0**level)  # length 차원 추가
            anchors.append(torch.concat([grid_xy, wh, length], -1).reshape(-1, height * width, 5))  # 4->5
        # define the valid range for anchor coordinates
        eps = 1e-2
        anchors = torch.concat(anchors, 1)
        valid_mask_xy_wh = ((anchors[..., :4] > eps) * (anchors[..., :4] < 1 - eps)).all(-1, keepdim=True)
        valid_mask_length = (anchors[..., 4:5] > 0).all(-1, keepdim=True)  # length는 0보다만 크면 됨
        valid_mask = valid_mask_xy_wh & valid_mask_length
        anchors_xy_wh = torch.log(anchors[..., :4] / (1 - anchors[..., :4]))
        anchors_length = torch.log(anchors[..., 4:5])  # length는 단순 log 변환
        anchors = torch.concat([anchors_xy_wh, anchors_length], dim=-1)
...
class RTDetrForObjectDetection(RTDetrPreTrainedModel):
    ...
    def __init__(self, config: RTDetrConfig):
        ...
        self.bbox_embed = partial(RTDetrMLPPredictionHead, config, config.d_model, config.d_model, 5, num_layers=3)
        ...
        self._loss_function = RTDetrForObjectDetectionLossWithHeight
