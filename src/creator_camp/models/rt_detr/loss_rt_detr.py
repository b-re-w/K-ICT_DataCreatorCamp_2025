...
from transformers.utils import is_scipy_available, is_vision_available, requires_backends
...
class RTDetrHungarianMatcher(nn.Module):
    ...
    def __init__(self, config):
        ...
        # length dimension에 대한 cost 추가
        self.length_cost = getattr(config, 'matcher_length_cost', 1.0)  # 기본값 1.0
    ...
    def forward(self, outputs, targets):
        ...
        # 5D bbox에 대한 cost 계산
        # 1. 전체 5D bbox에 대한 L1 cost
        bbox_cost = torch.cdist(out_bbox, target_bbox, p=1)

        # 2. 4D bbox (cx, cy, w, h)에 대한 GIoU cost
        out_bbox_4d = out_bbox[:, :4]
        target_bbox_4d = target_bbox[:, :4]
        giou_cost = -generalized_box_iou(
            center_to_corners_format(out_bbox_4d),
            center_to_corners_format(target_bbox_4d)
        )

        # 3. length dimension에 대한 별도 cost
        out_length = out_bbox[:, 4:5]
        target_length = target_bbox[:, 4:5]
        length_cost = torch.cdist(out_length, target_length, p=1)
...
class RTDetrLoss(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.weight_dict = {
            ...
            "loss_length": getattr(config, 'weight_loss_length', 2.0),  # length loss weight 추가
        }
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # IoU는 4D bbox (cx, cy, w, h)만 사용하여 계산
        src_boxes_4d = src_boxes[:, :4]
        target_boxes_4d = target_boxes[:, :4]
        ious, _ = box_iou(
            center_to_corners_format(src_boxes_4d.detach()),
            center_to_corners_format(target_boxes_4d)
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        Modified to handle 5D bboxes with separate length loss.
        """
        if "pred_boxes" not in outputs:
            raise KeyError("No predicted boxes found in outputs")
        idx = self._get_source_permutation_idx(indices)
        ...
        # 1. 전체 5D bbox에 대한 L1 loss
        loss_bbox = F.l1_loss(src_boxes, target_boxes, reduction="none")
        losses["loss_bbox"] = loss_bbox.sum() / num_boxes

        # 2. 4D bbox (cx, cy, w, h)에 대한 GIoU loss
        src_boxes_4d = src_boxes[:, :4]
        target_boxes_4d = target_boxes[:, :4]
        loss_giou = 1 - torch.diag(
            generalized_box_iou(
                center_to_corners_format(src_boxes_4d),
                center_to_corners_format(target_boxes_4d)
            )
        )
        losses["loss_giou"] = loss_giou.sum() / num_boxes

        # 3. length dimension에 대한 별도 loss (L1 또는 L2)
        src_length = src_boxes[:, 4]
        target_length = target_boxes[:, 4]
        loss_length = F.l1_loss(src_length, target_length, reduction="none")  # 또는 F.mse_loss
        losses["loss_length"] = loss_length.sum() / num_boxes
